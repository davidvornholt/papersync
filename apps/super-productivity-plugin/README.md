...
Click the “Import homework” header button to fetch approved homework, choose a project, review the subject tags, and import. Each subject automatically selects an existing tag with the same name, ignoring case and surrounding spaces. Missing or duplicate matches require choosing a tag or “No tag”; you can also change an automatic match. These choices apply to that import only. Cancel leaves the queue untouched. The plugin makes no background checks. Existing tasks keep their project and tags when a repeated scan updates their due date. Keys stay in Super Productivity’s local secret storage. Replacing or revoking a key in PaperSync invalidates the old connection.

Tasks carry a PaperSync ID in their notes. A retry finds that ID in active or archived tasks before creating anything. A reviewed due date, including its removal, is applied to the matching active task. Completion is managed in Super Productivity, and archived tasks stay archived. Keep the marker in the task notes if you want retries to recognize the task. Edited wording on a later scan creates a distinct task, so check the review before approving it.
