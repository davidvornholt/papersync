...
`bun run check:fix` at the root runs the full gate, including isolated PostgreSQL queue checks and authenticated desktop and mobile accessibility checks. Start the development database before running it. The second browser pass sets a fixture `GOOGLE_VERTEX_PROJECT` to verify managed Settings and inaccessible browser credentials; it makes no live model calls. Browser checks create signed test sessions using isolated fixture credentials; they do not bypass authorization. Live GitHub OAuth and paid OCR providers require their real credentials and are not simulated as successful live integrations.
